...
JOIN `Bookings` AS B ON B.memid = M.memid
JOIN `Facilities` AS F ON F.facid = B.facid
WHERE SUBSTRING(B.starttime, 1, 10) = '2012-09-14'
...
(SELECT F.name AS Facility, CONCAT(M.firstname, ' ', M.surname) AS Member, B.starttime AS time_,
CASE WHEN M.memid = 0 THEN B.slots * F.guestcost
ELSE B.slots * F.membercost END AS cost
FROM  `Members` AS M
JOIN `Bookings` AS B ON B.memid = M.memid
JOIN `Facilities` AS F ON F.facid = B.facid) AS BOOKINGS
WHERE SUBSTRING(time_, 1, 10) = '2012-09-14'
AND Cost > 30
ORDER BY Cost DESC
...
SELECT Facility, SUM(Cost) AS REVENUE FROM
(SELECT 
F.name AS Facility,
CASE WHEN M.memid = 0 THEN B.slots * F.guestcost
ELSE B.slots * F.membercost END AS cost
